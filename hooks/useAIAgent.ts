...
export function useAIAgent() {
  ...
  useEffect(() => {
    ...
    console.log("🔄 [AI Agent Hook] History updated, length:", conversationHistory.length);
  }, [conversationHistory]);
  ...
  const sendMessage = useCallback(async (transcript: string): Promise<AgentResponse | null> => {
    ...
    console.log("📚 [AI Agent Hook] History length:", historyRef.current.length);
    ...
    try {
      ...
      showToast.error(error.message || "Failed to process message");
      setIsLoading(false);
      return null;
    }
  }, [playAudio]);
